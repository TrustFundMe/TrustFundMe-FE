import { NextRequest, NextResponse } from 'next/server';
import { isJwtExpired } from '@/lib/utils';

/**
 * Update user avatar URL
 * Syncs avatar URL with BE using PUT /api/users/{userId}
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { userId, avatarUrl } = body;

    if (!userId) {
      return NextResponse.json(
        { error: 'User ID is required' },
        { status: 400 }
      );
    }

    if (!avatarUrl) {
      return NextResponse.json(
        { error: 'Avatar URL is required' },
        { status: 400 }
      );
    }

    const BE_API_URL = process.env.BE_API_GATEWAY_URL || 'http://localhost:8080';
    const accessToken = request.cookies.get('access_token')?.value;
    if (!accessToken) {
      console.error('[avatar] No access_token in cookies');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const beUrl = `${BE_API_URL}/api/users/${userId}`;
    const putBody = { avatarUrl };
    const response = await fetch(beUrl, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${accessToken}`,
      },
      body: JSON.stringify(putBody),
    });

    if (!response.ok) {
      const err = await response.json().catch(() => ({}));
      console.error('[avatar] BE PUT %s failed: status=%s, hasToken=%s, body=%o', beUrl, response.status, !!accessToken, err);

      let errorMsg: string;
      if (response.status === 401) {
        const expired = isJwtExpired(accessToken);
        errorMsg = expired
          ? 'Your session has expired. Please sign in again.'
          : 'Session invalid. Please sign in again. If it persists, run API Gateway with run-api-gateway.ps1 so JWT_SECRET is loaded from .env.';
      } else {
        errorMsg = (err as { message?: string }).message || (err as { error?: string }).error || 'Failed to update avatar';
      }

      return NextResponse.json({ error: errorMsg, avatarUrl }, { status: response.status });
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error updating avatar:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
  }
}
