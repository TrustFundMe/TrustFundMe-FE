...
import type { FlagRequest, FlagTargetType, RequestStatus } from '@/components/staff/request/RequestTypes';
import { mockFlagRequests } from '@/components/staff/request/mock';
...
export default function StaffFlagRequestsPage() {
  const [rows, setRows] = useState<FlagRequest[]>(mockFlagRequests);
  ...
  return (
    <section className="space-y-4">
      <div>
        <div className="text-lg font-semibold text-gray-900">Flag reports</div>
        <div className="mt-1 text-sm text-gray-500">Review flagged content and decide actions.</div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4">
          <RequestDetailPanel
            request={selected}
            title={selected ? `Flag · ${targetLabel[selected.targetType]}` : 'Report details'}
            fields={[
              { label: 'Created at', value: selected?.createdAt },
              { label: 'Target', value: selected ? `${targetLabel[selected.targetType]} · ${selected.targetId}` : undefined },
              { label: 'Reporter', value: selected?.reporterName },
              { label: 'Reason', value: selected?.reason },
              { label: 'Preview', value: selected?.previewText || '-' },
            ]}
