...
export default function DisableCampaignModal({ isOpen, onClose, onConfirm, campaignTitle }: DisableCampaignModalProps) {
        ...
        >
          ...
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-bold text-gray-900">Khóa chiến dịch</h3>
            </div>
            <button
              onClick={onClose}
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-start gap-3 p-3 bg-red-50 rounded-xl border border-red-100">
                ...
                <p className="text-sm text-red-800 leading-relaxed font-medium">
                  Bạn có chắc chắn muốn khóa chiến dịch <span className="font-bold">"{campaignTitle}"</span>? 
                  Chiến dịch sẽ ngừng tiếp nhận quyên góp và không còn hiển thị công khai.
                </p>
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Lý do khóa chiến dịch <span className="text-red-500">*</span>
                </label>
                <textarea
                  required
                  value={reason}
                  onChange={(e) => setReason(e.target.value)}
                  placeholder="Nhập lý do chi tiết để thông báo cho chủ sở hữu..."
                  rows={4}
