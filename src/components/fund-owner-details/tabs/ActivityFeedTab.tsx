...
/**
 * Parse raw target name like "evidence|37" into a human-readable label.
 * Returns "Minh chứng chi tiêu #37" for "evidence|37",
 * "Chiến dịch #5" for "campaign|5", etc.
 * If no pipe delimiter, returns the original string.
 */
const parseTargetName = (raw: string | undefined | null): string => {
    ...
    if (parts.length === 2) {
        ...
        const suffix = isNumericId ? ` #${id}` : `: ${id}`;
        switch (type) {
            case 'evidence':
                return `Minh chứng chi tiêu${suffix}`;
            case 'campaign':
                return `Chiến dịch${suffix}`;
            case 'expenditure':
                return `Khoản chi tiêu${suffix}`;
            default:
                return raw;
        }
    }
...
const FeedItem = ({ post }: { post: FeedPostDto }) => {
    const getIcon = () => {
        if (post.targetType === 'EXPENDITURE') return <ShieldCheck size={14} />;
        if (post.targetType === 'CAMPAIGN') return <Megaphone size={14} />;
    ...
    const parsedTag = parseTargetName(post.targetName);
    const hasTag = parsedTag !== '';
    ...
    return (
        <div className="feed-item">
            ...
            <style jsx>{`
        .feed-item { display: flex; gap: 16px; font-family: var(--font-dm-sans, 'DM Sans', 'Inter', sans-serif); }
        .feed-left { display: flex; flex-direction: column; align-items: center; }
        .icon-circle { 
            width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center;
            border: 2px solid transparent; flex-shrink: 0; color: #0f172a;
        }
        .icon-circle.emergency { background: #fff3ed; border-color: #ffe4d4; color: #ff5e14; }
        .icon-circle.evidence { background: #f0fdf4; border-color: #dcfce7; color: #059669; }
        .icon-circle.update { background: #f8fafc; border-color: rgba(15,23,42,0.10); color: #0f172a; }
        .line { width: 1.5px; flex: 1; background: rgba(15,23,42,0.10); margin: 4px 0; }
        ...
        .campaign-tag { 
            ...
            background: #fff3ed; border: 1px solid #ffe4d4; border-radius: 6px;
            font-size: 11px; font-weight: 700; color: #ff5e14;
        }
        .feed-date { font-size: 11px; color: #0f172a; font-weight: 500; opacity: 0.5; }
        
        .feed-body { background: #fff; border: 1px solid rgba(15,23,42,0.10); border-radius: 12px; padding: 20px; margin-bottom: 12px; transition: box-shadow 0.2s; }
        .feed-body:hover { box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.05); }
        .post-title { margin: 0 0 8px 0; font-size: 14px; font-weight: 800; color: #0f172a; }
        .text { font-size: 13px; color: #0f172a; line-height: 1.6; margin: 0; }
        ...
        .post-img { 
            ...
            border-radius: 8px; border: 1px solid rgba(15,23,42,0.10);
        }

        .view-details, :global(.view-details) { 
            display: inline-flex; align-items: center; gap: 4px;
            background: none; border: none; color: #ff5e14; font-size: 10px; font-weight: 800; 
            padding: 0; cursor: pointer; text-decoration: none; text-transform: uppercase; letter-spacing: 0.5px;
        }
        .view-details:hover, :global(.view-details:hover) { text-decoration: underline; }
      `}</style>
        </div>
    );
};
...
const CommentItem = ({ comment }: { comment: CommentDto }) => {
    ...
    const getPostIcon = (type: string | undefined | null) => {
        if (type === 'EXPENDITURE') return <ShieldCheck size={10} />;
        if (type === 'CAMPAIGN') return <Megaphone size={10} />;
        return <MessageSquareMore size={10} />;
    };

    const parsedPostTag = parseTargetName(comment.postTargetName);
    ...
    return (
        <div className="comment-item-container">
                ...
                {isReply ? (
                    ...
                    <div className="post-mini">
                        <div className="post-mini-header">
                            ...
                            <div className="post-tag-mini">{parsedPostTag || comment.postTargetName}</div>
                        </div>
            ...
            <style jsx>{`
                .comment-item-container { margin-bottom: 12px; padding-bottom: 4px; font-family: var(--font-dm-sans, 'DM Sans', 'Inter', sans-serif); }
                
                .context-card { background: #fff; border: 1px solid rgba(15,23,42,0.10); border-radius: 10px; padding: 12px; margin-left: 16px; border-left: 3px solid #ff5e14; }
                ...
                .mini-name { font-size: 11px; font-weight: 700; color: #0f172a; }
                .mini-content { font-size: 11px; color: #0f172a; line-height: 1.4; margin: 0; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
                
                .post-mini-header { display: flex; align-items: center; gap: 6px; margin-bottom: 6px; }
                .post-tag-mini { font-size: 9px; font-weight: 800; color: #ff5e14; text-transform: uppercase; background: #fff3ed; padding: 2px 6px; border-radius: 4px; }
                .post-mini-title { font-size: 11px; font-weight: 800; color: #0f172a; margin: 0 0 4px 0; }
                .post-mini-meta { font-size: 9px; color: #0f172a; opacity: 0.5; display: flex; align-items: center; gap: 4px; }
                
                .owner-comment-row { display: flex; gap: 16px; margin-top: -4px; }
                .connector-line { width: 2px; background: rgba(15,23,42,0.10); margin-left: 28px; height: 20px; flex-shrink: 0; }
                ...
                .owner-name { font-size: 12px; font-weight: 800; color: #0f172a; }
                .owner-date { font-size: 10px; color: #0f172a; opacity: 0.5; }
                
                .owner-bubble { background: #fff3ed; border-radius: 12px; border-top-left-radius: 2px; padding: 10px 14px; }
                .owner-bubble p { font-size: 12px; color: #0f172a; line-height: 1.5; margin: 0; }
            `}</style>
        </div>
    );
};
...
const ActivityFeedTab = ({ id }: { id: string | number }) => {
    ...
    return (
        <div className="activity-tab-container">
                    ...
                    <div className="comments-list">
                        {loadingComments ? (
                            <div className="loading-state-mini">
                                <Loader2 className="animate-spin" size={24} style={{ color: '#ff5e14' }} />
                            </div>
                        ) : comments.length > 0 ? (
            ...
            <style jsx>{`
        .activity-tab-container { padding: 32px; background: #fff; height: calc(100vh - 250px); overflow: hidden; display: flex; flex-direction: column; font-family: var(--font-dm-sans, 'DM Sans', 'Inter', sans-serif); }
        .activity-grid { display: grid; grid-template-columns: 1.2fr 1fr; gap: 48px; width: 100%; height: 100%; min-height: 0; margin: 0 auto; }
        
        .posts-column { border-right: 1px solid rgba(15,23,42,0.10); padding-right: 32px; height: 100%; overflow-y: auto; }
        ...
        .filter-btn { 
          padding: 6px 14px; border-radius: 99px; border: 1px solid rgba(15,23,42,0.15); 
          background: #fff; font-size: 11px; font-weight: 600; color: #0f172a; cursor: pointer;
          transition: all 0.2s; font-family: inherit;
        }
        .filter-btn:hover { background: #fff3ed; border-color: #ff5e14; color: #ff5e14; }
        .filter-btn.active { background: #ff5e14; color: #fff; border-color: #ff5e14; }
        
        .feed-list { display: flex; flex-direction: column; gap: 12px; }
        .loading-state, .empty-state { padding: 40px; text-align: center; color: #0f172a; font-size: 14px; display: flex; justify-content: center; align-items: center; height: 200px; }
        ...
        .column-header { margin-bottom: 24px; border-bottom: 2px solid rgba(15,23,42,0.06); padding-bottom: 12px; position: sticky; top: 0; background: #fff; z-index: 10; }
        .column-header h3 { font-size: 16px; font-weight: 800; color: #0f172a; margin: 0; }
        ...
        .posts-column::-webkit-scrollbar-thumb, .comments-column::-webkit-scrollbar-thumb { background: rgba(15,23,42,0.10); border-radius: 10px; }
        .posts-column:hover::-webkit-scrollbar-thumb, .comments-column:hover::-webkit-scrollbar-thumb { background: rgba(15,23,42,0.20); }
        ...
        @media (max-width: 992px) {
            ...
            .posts-column { border-right: none; padding-right: 0; border-bottom: 1px solid rgba(15,23,42,0.10); padding-bottom: 24px; margin-bottom: 24px; height: auto; overflow: visible; }
