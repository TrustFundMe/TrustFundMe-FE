...
Test-Endpoint -Name "Backend Check Email" -Url "http://116.118.9.149:8080/api/users/check-email?email=$email" -Method "GET"

# 2. Test Frontend Check Email (Proxy)
Write-Host "`n--- 2. Testing Frontend Proxy (Port 80) ---" -ForegroundColor Magenta
$feCheck = Test-Endpoint -Name "Frontend Check Email" -Url "http://116.118.9.149/api/auth/check-email" -Method "POST" -Body @{ email = $email }
...
Test-Endpoint -Name "Frontend Send OTP" -Url "http://116.118.9.149/api/auth/send-otp" -Method "POST" -Body @{ email = $email }

# 4. Test Backend Send OTP (Direct)
Write-Host "`n--- 4. Testing Backend Send OTP (Direct - Port 8080) ---" -ForegroundColor Magenta
Test-Endpoint -Name "Backend Send OTP" -Url "http://116.118.9.149:8080/api/auth/send-otp" -Method "POST" -Body @{ email = $email }
